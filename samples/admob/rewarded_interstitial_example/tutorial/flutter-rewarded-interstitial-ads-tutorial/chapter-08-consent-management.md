...
对于面向欧洲经济区（EEA）和英国用户的应用，必须遵守 GDPR（通用数据保护条例）要求。Google 提供了 User Messaging Platform (UMP) 来帮助开发者收集和管理用户同意。本章将详细讲解如何集成 UMP 并实现用户同意管理。
