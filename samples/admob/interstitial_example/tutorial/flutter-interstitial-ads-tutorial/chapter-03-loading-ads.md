...
在本章中，我们将学习如何使用 Google Mobile Ads SDK 从广告服务器加载 Interstitial 广告。我们将详细讲解 `InterstitialAd.load()` 方法、`InterstitialAdLoadCallback` 回调处理、成功和失败场景，以及错误处理机制。

## 广告加载流程

加载 Interstitial 广告的基本流程如下：

1. **调用加载方法**：使用 `InterstitialAd.load()` 方法请求广告
...
#### InterstitialAd.load() 方法

`InterstitialAd.load()` 是 Google Mobile Ads SDK 提供的静态方法，用于加载 Interstitial 广告。它接受以下参数：

- `adUnitId`：广告单元 ID（字符串）
- `request`：`AdRequest` 对象，包含广告请求的配置信息
- `adLoadCallback`：`InterstitialAdLoadCallback` 对象，处理加载结果
...
对于 Interstitial Ads，通常使用默认的 `AdRequest()` 即可。

#### InterstitialAdLoadCallback

`InterstitialAdLoadCallback` 是一个回调类，包含两个回调函数：

- `onAdLoaded`：广告加载成功时调用，接收加载的 `InterstitialAd` 对象作为参数
...
- `ad` 参数是成功加载的 `InterstitialAd` 对象
- 使用 `debugPrint` 记录加载成功的日志（仅在调试模式下输出）
- 将加载的广告对象保存到 `_interstitialAd` 变量中
...
```text
InterstitialAd failed to load: LoadAdError(...)
Error code: 3
...
在显示广告之前，检查 `_interstitialAd` 是否为 `null`：
...
- [ ] 理解广告加载的基本流程
- [ ] 实现 `_loadAd()` 方法使用 `InterstitialAd.load()`
- [ ] 处理 `onAdLoaded` 成功回调
...
现在我们已经实现了广告加载功能，在下一章中，我们将学习如何显示已加载的广告，包括显示前的检查和资源管理。

继续学习：[第 4 章：显示插屏广告](chapter-04-showing-ads.md)
