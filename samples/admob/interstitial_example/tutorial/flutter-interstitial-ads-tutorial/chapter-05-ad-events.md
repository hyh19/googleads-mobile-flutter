...
在本章中，我们将学习如何监听和处理 Interstitial 广告的各种事件。`FullScreenContentCallback` 提供了多个回调函数，用于处理广告展示、关闭、点击等事件。我们将详细讲解每个回调函数的用途和实现方法。

## FullScreenContentCallback 概述

### FullScreenContentCallback 类

`FullScreenContentCallback` 是一个回调类，用于处理全屏广告（包括 Interstitial Ads）的各种生命周期事件。它包含多个可选的回调方法，你只需要实现你关心的事件。
...
- `ad` 参数是被关闭的 `InterstitialAd` 对象
- 调用 `dispose()` 释放广告资源
- 清空 `_interstitialAd` 引用
- **重要**：立即加载下一个广告，为下次显示做准备

**为什么立即加载下一个广告？**

Interstitial Ads 需要预加载才能在需要时立即显示。如果等到需要显示时再加载，用户会看到明显的延迟，影响用户体验。
...
```dart
onAdShowedFullScreenContent: (ad) {
  debugPrint('Ad showed - pausing video');
  _videoPlayerController.pause();
},

onAdDismissedFullScreenContent: (ad) {
  debugPrint('Ad dismissed - resuming video');
...
Interstitial Ads 需要预加载才能在需要时立即显示。如果等到需要显示时再加载，用户会看到明显的延迟。立即加载下一个广告可以确保在用户下次需要时，广告已经准备好了。
...
现在我们已经学会了如何监听和处理广告事件。在下一章中，我们将学习如何选择合适的显示时机，这对于用户体验和广告效果都非常重要。

继续学习：[第 6 章：显示时机选择](chapter-06-timing.md)
